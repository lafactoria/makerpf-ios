...
var PFInstructions = function(listener){
	...
	//=======================================================================
	function loadInstructions(folder){
		this.folder = folder;
	...
	function showPopupInstructions(evt){
		...
		if((typeof(text) == 'undefined' || text == "") && (typeof(image) != 'undefined' && image != "")){							
			...
			var imgPopup = new Bitmap("game/"+folder+"/images/"+image);
			imgPopup.image.onload = function(){
				scaleImg 		= getImageScale(imgPopup.image.width,imgPopup.image.height,350);
				imgPopup.scaleX = scaleImg;
				imgPopup.scaleY = scaleImg;
				imgPopup.x 		= (512-((imgPopup.image.width*scaleImg)/2));				
				imgPopup.y 		= (230-((imgPopup.image.height*scaleImg)/2));
				container.addChild(imgPopup);	
			}
			...
			fletxaPopup.x = 400 + ((bgPopUpBitmap.image.width/2)*0.84);
	...
	function closeInstructions(evt){
		Tween.get(evt.target.container).to({alpha:0},600).call(function(){
				baseGame.onStartGame();
		});
	}
	//=====================================================
	// REPEAT POPUP SOUND
